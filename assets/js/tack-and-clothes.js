document.addEventListener('DOMContentLoaded', () => {
    ...
    sidebarContent.innerHTML = '<p>Click on a tack item to see its details</p>';
    ...
    // Create quiz mode toggle button
    ...
    const groupData = Array.from(groups).map(group => {
        ...
        const descEl = group.querySelector('desc');
        return {
            id: group.id,
            title: titleEl?.textContent || group.id || 'Unknown',
            desc: descEl?.textContent || 'No description available',
            originalTitle: titleEl?.textContent || group.id || 'Unknown',
            matched: false,
            correct: false
        };
    });
    ...
    const resetGroupStyle = (group) => {
        ...
        paths.forEach((p, i) => {
            ...
            p.style.fillOpacity = isQuizMode && groupData.find(g => g.id === group.id)?.matched ? '0.3' : styles[i].fillOpacity;
            p.style.fill = styles[i].fill;
        });
    };
    ...
    modeToggleButton.addEventListener('click', () => {
        ...
        if (isQuizMode) {
            ...
            clickedGroup = document.querySelector('#drop-blanket');
    ...
    groups.forEach((group, index) => {
        ...
        group.addEventListener('mouseover', (e) => {
            if (titleEl) {
                titleEl.dataset.originalTitle = groupData.find(g => g.id === group.id).originalTitle;
                titleEl.textContent = '';
            }
            if (!isQuizMode) {
    ...
    // Set default layer (blanket) on load
    const defaultGroup = document.querySelector('g#drop-blanket');
